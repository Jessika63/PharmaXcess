...
# GET UNIQUE
@diseases_bp.route("/disease/<int:disease_id>", methods=["GET"])
...
# UPDATE
@diseases_bp.route("/disease/<int:disease_id>", methods=["PUT"])
def update_disease(disease_id):
    ...
    with conn.cursor() as cursor:
        cursor.execute("""
            ...
            WHERE id=%s
        """, (data.get("nom"), data.get("description"), data.get("symptomes"), data.get("date_debut"), disease_id))
        conn.commit()
...
# DELETE
@diseases_bp.route("/disease/<int:disease_id>", methods=["DELETE"])
