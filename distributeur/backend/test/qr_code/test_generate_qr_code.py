...
def test_generate_prescription_qr_success(client):
    ...
    with patch('routes.qr_code.generate_qr_code.generate_rounded_qr_code') as mock_generate:
        ...
        response = client.post('/generate_prescription_qr', 
...
def test_generate_direction_qr_success(client):
    ...
    with patch('routes.qr_code.generate_qr_code.generate_rounded_qr_code') as mock_generate:
        ...
        response = client.post('/generate_direction_qr', 
...
@pytest.mark.order(1)
def test_cleanup_qr_files():
    """
    Clean up any QR code files created during testing
    """
    import os
    import glob
    
    # Remove any prescription_*.png files
    for file in glob.glob("prescription_*.png"):
        os.remove(file)
...
def test_generate_rounded_qr_code_return_buffer(mock_env_file):
    ...
    # Mock the encryption and QR code creation to avoid dependencies
    with patch("scripts.qrcode.qrCodeGen.encrypt_data") as mock_encrypt:
        mock_encrypt.return_value = "encrypted_data"
...
def test_generate_rounded_qr_code_env_file_invalid():
    ...
    with patch("os.path.exists", return_value=True):
        with patch("builtins.open", mock_open(read_data="INVALID_LINE_WITHOUT_EQUALS")):
