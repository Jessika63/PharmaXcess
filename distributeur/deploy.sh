...
FRONT_PATH="./frontend"                 # Local path to frontend
...
ssh "$RPI_USER@$RPI_IP" "mkdir -p ~/build/distributeur/frontend"
scp -r "$FRONT_PATH/build/"* "$RPI_USER@$RPI_IP:~/build/distributeur/frontend/"
...
docker run -d --name $CONTAINER_NAME -p $DOCKER_PORT:80 \
    -v ~/build/distributeur/frontend:/usr/share/nginx/html:ro \
    nginx
"
